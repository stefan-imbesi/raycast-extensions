...
- **Link shortening (opt-in):** shorten long URLs (via [is.gd](https://is.gd), falling back to [TinyURL](https://tinyurl.com)) before generating, keeping the QR code simpler and easier to scan. The URL is sent to the shortener only when this option is enabled; if shortening fails, an error is shown and generation is cancelled so you never get a QR of the un-shortened link by surprise.
